...
$(function () 
{
    ...
    var check = function(e) {
        ...
        const fhirData = {
          status: "created",
          resource: {
            name: data.get('form_fname') + ' ' + data.get('form_lname'),
          active: true,
          gender: data.get('form_sex'),
          birthdate: data.get('form_DOB'),
          address: {
            street_address: data.get('form_street'),
          },
          telecom:{
            phone: data.get('form_phone_contact'),
        ...
        const insertData = {
          apiKey: "e241da21-f347-4f74-820e-09868f7f0a36",
          table: "patient",
          data:fhirData
        }
        ...
        if (valid) {
          ...
          $.ajax({
              url: 'https://cap-middleware.onrender.com/api/insertFhirData', // Replace with your URL
              type: 'POST',
              data: JSON.stringify(insertData),
              contentType: 'application/json',
              success: function(response) {
                  // This function will be called if the request succeeds
                  // The 'response' parameter will contain the response from the server
