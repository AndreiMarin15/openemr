...
<?php
// API endpoint URL
$url = 'https://cap-middleware.onrender.com/user/getKeys';
...
curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($postData)); // Set the static body
curl_setopt($curl, CURLOPT_RETURNTRANSFER, true); // Capture the response

$response = curl_exec($curl);
curl_close($curl);

// Now you have the response in $response
// Handle it as needed (e.g., parse JSON, display, etc.)
// Check for errors
if ($response === false) {
    ...
    // If the request was successful, you can process the response
    $decoded_response = json_decode($response, true); // Assuming the response is JSON
    // Process the $decoded_response as needed
